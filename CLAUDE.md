...
- **Колапс матерії:** вимагає енергію на капі (`1e308`); **повний ресет Тіру 0** — енергія → стартова, рівні генераторів → 0, рівні **всіх апгрейдів → 0**, **Кристали Пустоти (VC) → 0** — і дає +1 обраної частинки (p/n/e). Повторювана дія, інкрементує `save.matterCollapses`.
  ...
  - **⚠️ Жива перевірка (V17) виявила серйозну напругу:** оскільки повний ресет забирає й VC, а VC (через CORE) — єдине, що дозволяє продукції пробити природну стелю `log10(E)≈23-24` (розділ 7.4), **повторний** колапс матерії (2-й і далі) практично недосяжний за розумний час — бот-симуляція застрягла на тій самій стелі ~23-24 навіть після ~65 ігрових днів підйому з нуля, так і не наблизившись до 1e308. Тобто "10 колапсів до Break Infinity" наразі реалістично означає: 1 колапс одразу (використовуючи багатство від попередніх престижів, VC ще не скинуто) + астрономічно довге очікування перед кожним наступним. Break Infinity в цьому проході живою перевіркою не підтверджено (не вдалось накопичити навіть 2 колапси за тестове вікно). **Не виправлено** — рішення (не скидати VC повністю при колапсі, чи знизити вимогу колапсу для 2-го+ разу, чи прийняти як свідомий надто-довгий темп) лишено для профільного рефайнменту, див. `docs/balance.md`.
