...
│   │   │   │   ├── CollapseCycleBonus.java    # буст від кількості колапсів матерії, незалежний від VC
...
**Цикл-буст колапсів (`CollapseCycleBonus`).** Той самий tanh-softcap патерн, що й `CORE` (жорстка асимптота, не sqrt-сповільнення), але вхід — `log10(save.matterCollapses + 1)` замість `log10(VC)`:
```
rawExponent = 2.5 * log10(matterCollapses + 1)
якщо rawExponent > 10: effectiveExponent = 10 + 10 * tanh((rawExponent-10) / 10)   // стеля ~1e20
інакше: effectiveExponent = rawExponent
boost = 10^effectiveExponent
```
На відміну від `CORE`, повністю НЕ залежить від VC — це навмисний, незалежний "місток" прогресу через Тір 1: жива бот-симуляція (`docs/balance.md`, V17) показала, що без VC виробництво природно впирається в стелю `log10(E)≈23-24`, роблячи повторний колапс матерії практично недосяжним (розділ 7.6). `matterCollapses` ніколи не скидається (на відміну від VC при колапсі), тож цей буст — єдине, що гарантовано росте з кожним циклом. Живе значення показує UI (`/api/matter-info` → `cycleBoost`, картка "Цикл колапсів" на сторінці "Грейди", matter.js).
...
Формула кристалів пустоти (константи в `PrestigeService`, з V17 — жива бот-симуляція, `docs/balance.md`):
```
log10Energy = exponent + log10(number)
якщо < PRESTIGE_MIN_LOG10_ENERGY (18.0) → 0 кристалів
base_log10 = (log10Energy - 18.0) / PRESTIGE_DIVISOR (2.5) + 1
...
**Жива перевірка (V17):** до V17 поріг 25.0/3.0 виявився фактично недосяжним — бот-симуляція (жадібне реінвестування, прискорений tick-speed) впиралась у природну стелю `log10(E)≈23-24` навіть після ~120 безперервних ігрових днів, бо до першого престижу єдиний мультиплікативний двигун (`CORE`) не діє без VC (`10^(...*log10(VC))`, VC=0). Поріг 18.0 навмисно обраний НИЖЧЕ цієї стелі. Результат: перший престиж досяжний за ~1.85 ігрового дня замість "ніколи". **Побічний ефект (виправлено V18):** щойно з'являється VC, `CORE` вмикав вибухове зростання — 2-й/3-й/4-й престиж у тому ж тесті пройшли за секунди кожен (4-й вже вдарився у стіну 1e308), що суперечило цілі "кожна наступна реінкарнація довша". V18 підняв `cost_multiplier` CORE (2.2 → 3.2) і зменшив `effect_value` (0.10 → 0.05) — темперує сніжний ком з обох боків одразу; ще не перевірено живою бот-симуляцією повторно (потребує наступного проходу).
...
- **Колапс матерії:** вимагає енергію на капі (`1e308`); **повний ресет Тіру 0** — енергія → стартова, рівні генераторів → 0, рівні **всіх апгрейдів → 0** — і дає +1 обраної частинки (p/n/e). Повторювана дія, інкрементує `save.matterCollapses` (ніколи не скидається).
  - На відміну від престижу (7.4), де апгрейди й VC навмисно **не** скидаються (це і є суть prestige-циклу), колапс матерії обнуляє все, крім частинок p/n/e і (з V18) **Кристалів Пустоти після `CollapseCycleBonus.VC_PERSISTS_AFTER_COLLAPSES` (10 000) колапсів**.
  - **⚠️ Жива перевірка (V17) виявила серйозну напругу, виправлено (V18):** повний ресет VC при КОЖНОМУ колапсі робив повторний колапс матерії практично недосяжним — VC (через CORE) було єдиним способом пробити природну стелю `log10(E)≈23-24` без нього. Виправлення двобічне: (1) `CollapseCycleBonus` — новий, незалежний від VC буст від самої кількості колапсів (7.1), дає альтернативний шлях назад до 1e308; (2) VC більше **не скидається** колапсом після 10 000 циклів — гравець "довів" накопичений досвід і отримує право на постійний VC/CORE-снігова-ком. До 10 000-го колапсу VC й далі скидається щоразу, як і апгрейди/генератори — саме `CollapseCycleBonus` несе прогрес до того моменту. Ще не перевірено живою бот-симуляцією (потребує наступного проходу).
...
| GET | `/api/matter-info/{saveId}` | Прапори Тіру 1, частинки, готовність до колапсу, живі бонуси частинок (`protonEnergyMult`/`neutronCostReduction`/`electronCrystalMult` з `ParticleBonus`), цикл-буст (`cycleBoost`/`vcPersistsAfterCollapses` з `CollapseCycleBonus`) |
...
| 12 | Баланс Tier 0 — живе бот-тестування (V17) виправило недосяжний поріг престижу; виявило й задокументувало (не виправило) снігову кулю CORE після 1-го престижу і недосяжність повторного колапсу матерії — потрібен профільний рефайнмент | 🔶 |
